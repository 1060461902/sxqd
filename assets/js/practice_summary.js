    ...
    option.success = function (data) {
        if (data.code === 200) {
            $('.report-show-body').handlebars($('#practice-summary-model'), data.data, [{
                name: 'show_info',
                callback: function (status_code, id, write_flag) {
                    var infos = status_code.split('_');
                    if (infos.length === 1) {
                        switch (status_code) {
                            case '1': //未到填写时段
                            ...
                            case '3': //已开始
                                if (write_flag === 1) {
                                    return '<a class="report-show-already" href="./practice_summary_check.html?id=' + id + '">' +
                ...
                callback: function (status_code, id) {
                    ...
                    switch (infos[0]) {
                        case '3':
                        case '4':
                            return '<a class="report-show-edit" href="./practice_summary_edit.html?id=' + id + '">' +
